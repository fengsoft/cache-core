...
export function createCacheCoreAdminApi(
	...
	function setRequestIdHeader(reply: FastifyReply, requestId: string) {
		reply.header("x-request-id", requestId);
	}

	function resolveRequestId(request: FastifyRequest) {
		const requestId = resolveHeaderValue(request.headers["x-request-id"]);
		return requestId && requestId.length > 0 ? requestId : crypto.randomUUID();
	}

	function resolveApiKey(request: FastifyRequest) {
		const authorization = resolveHeaderValue(request.headers.authorization);

		if (authorization?.startsWith("Bearer ")) {
			return authorization.slice("Bearer ".length).trim();
		}

		return resolveHeaderValue(request.headers["x-api-key"]) ?? "";
	}
	...
	function beginRequest(
		...
		const span = tracer.startSpan(`http ${request.method} ${route}`, {
			traceparent: resolveHeaderValue(request.headers.traceparent),
			attributes: {
				route,
	...
	function finalizeRequest(
		...
		const statusCode = resolveStatusCode(reply.statusCode);
		const durationMs =
	...
	function respondError(
		...
		setRequestIdHeader(reply, context.requestId);
		finalizeRequest(context, reply, {
			errorCode: code,
		});
		return body;
	}
	...
	function ensureMetricsAccessible(
		...
		return ensureAuthorized(context, request, reply);
	}
	...
	app.setErrorHandler((error, request, reply) => {
		...
		const context = beginRequest(route, request, reply);
		const failureReason =
	...
	app.get("/health", async (request, reply) => {
		const context = beginRequest("/health", request, reply);
	...
	app.get("/ready", async (request, reply) => {
		const context = beginRequest("/ready", request, reply);
	...
	app.get("/metrics", async (request, reply) => {
		const context = beginRequest("/metrics", request, reply);
		const unauthorized = ensureMetricsAccessible(context, request, reply);
	...
	app.post("/v1/invalidate", async (request, reply) => {
		const context = beginRequest("/v1/invalidate", request, reply);
		const unauthorized = ensureAuthorized(context, request, reply);

		if (unauthorized) {
			return unauthorized;
		}

		const parsed = invalidateRequestSchema.safeParse(request.body);
	...
	app.post("/v1/warm", async (request, reply) => {
		const context = beginRequest("/v1/warm", request, reply);
		const unauthorized = ensureAuthorized(context, request, reply);
		...
		const parsed = warmRequestSchema.safeParse(request.body);
	...
	app.get("/v1/metrics", async (request, reply) => {
		const context = beginRequest("/v1/metrics", request, reply);
		const unauthorized = ensureAuthorized(context, request, reply);
		...
		finalizeRequest(context, reply, snapshot);
		return snapshot;
	});
